...
begin_read_buff:
      ...
      else if (events[n].data.fd == tun_fd)
      {
        ...
        uint32_t *tun_buff_len_net = (uint32_t *)&tun_buff[0];
        uint32_t tun_buff_len;
        
        nread = read_buff2(tun_fd, tun_buff+sizeof(*tun_buff_len_net), BUFFSZ);
        if (nread < 0)
        {
        ...
        *tun_buff_len_net = htonl((uint32_t)nread);
        ...
        nwrite = write_buff(client_fd, tun_buff, tun_buff_len+sizeof(*tun_buff_len_net));
        if (nwrite < 0)
        {
          perror("write_buff(client_fd, buff, *tun_buff_len)");
          return -1;
        }
